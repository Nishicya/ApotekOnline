...
<div class="col-12 grid-margin stretch-card">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <p class="card-description">Form Penjualan Obat</p>

            <form class="forms-sample" action="{{ route('penjualan.store') }}" method="POST" id="form-penjualan" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            @php
                                $lastNota = App\Models\Penjualan::orderBy('id', 'desc')->first();
                                $no_nota = 'PJ-' . date('Ymd') . '-' . sprintf('%03d', ($lastNota ? $lastNota->id + 1 : 1));
                            @endphp
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="tgl_penjualan" name="tgl_penjualan" value="{{ date('Y-m-d') }}" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="id_pelanggan" name="id_pelanggan" required>
                                ...
                                <option value="{{ $p->id }}">{{ $p->nama_pelanggan }} - {{ $p->email }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="id_jenis_kirim" name="id_jenis_kirim" required>
                                ...
                                <option value="{{ $jk->id }}">{{ $jk->nama_ekspedisi }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="row mt-4">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="url_resep">Upload Resep (khusus obat keras)</label>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="ongkos_kirim" class="form-control" value="0" min="0" required>
                            @error('ongkos_kirim')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="number" name="biaya_app" class="form-control" value="0" min="0" required>
                            @error('biaya_app')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <a href="{{ route('penjualan.manage') }}" class="btn btn-light">Batal</a>
            </form>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    function calculateTotal() {
        ...
        var ongkir = parseFloat($('input[name="ongkos_kirim"]').val()) || 0;
        var biayaApp = parseFloat($('input[name="biaya_app"]').val()) || 0;
        
        var totalBayar = total + ongkir + biayaApp;
        $('input[name="total_bayar"]').val(totalBayar.toFixed(2));
    }
    ...
    // Event listener untuk ongkir dan biaya app
    $(document).on('input', 'input[name="ongkos_kirim"], input[name="biaya_app"]', function() {
        calculateTotal();
    });
